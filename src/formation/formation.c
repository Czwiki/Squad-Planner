 ...
 * ============================================================================
 * File format (text-based, one record per line):
 * 
 * PLAYER:<name>:<age>:<overall>:<potential>:<own_rating>
 * FORMATION:<name>
 * POSITION:<formation_name>:<position_id>
 * ASSIGN:<formation_name>:<position_id>:<player_name>
 * 
 * Lines starting with # are comments
 * ============================================================================
 */

#define SAVE_FILE_PATH "squad_planner_data.txt"
#define MAX_LINE_LENGTH 512

/**
 * @brief Save all data (players and formations) to a file.
 * 
 * Writes all players and formations to the default save file.
 * The file format is text-based for easy inspection and editing.
 * 
...
int save_data(void) {
    ...
    fprintf(file, "# Squad Planner Save File\n");
    fprintf(file, "# Do not edit manually unless you know the format\n\n");
    
    /* Save all players */
    fprintf(file, "# Players\n");
    player* p = player_head;
    while (p != NULL) {
        fprintf(file, "PLAYER:%s:%d:%d:%d:%d\n", 
    ...
    /* Save all formations */
    formation* f = formation_head;
    while (f != NULL) {
        fprintf(file, "FORMATION:%s\n", f->name);
        
        /* Save positions for this formation */
        for (int i = 0; i < 24; i++) {
            if (f->map_of_positions[i] != NULL) {
                fprintf(file, "POSITION:%s:%d\n", f->name, i);
                
                /* Save player assignments for this position */
                position* pos = f->map_of_positions[i];
                for (int j = 0; j < pos->size_of_list; j++) {
                    fprintf(file, "ASSIGN:%s:%d:%s\n", 
                            f->name, i, pos->list_of_players[j]->name);
                }
            }
        }
        f = f->next;
    ...
    if (fclose(file) != 0) {
        perror("Error closing save file");
        return -1;
    }
...
/**
 * @brief Load all data (players and formations) from a file.
 * 
 * Reads players and formations from the default save file.
 * Existing data is NOT cleared - loaded data is merged.
 * 
 * @return 0 on success, -1 on file/parse error
 */
int load_data(void) {
    ...
    while (fgets(line, MAX_LINE_LENGTH, file) != NULL) {
        ...
        if (strncmp(line, "PLAYER:", 7) == 0) {
            /* Parse player: PLAYER:name:age:overall:potential:own */
            char name[128];
            int age, overall, potential, own;
            
            if (sscanf(line + 7, "%127[^:]:%d:%d:%d:%d", 
                       name, &age, &overall, &potential, &own) != 5) {
                fprintf(stderr, "Warning: Invalid player line %d\n", line_num);
                continue;
            }
            ...
            /* Create player (reusing the args-based function) */
            char age_str[16], ovr_str[16], pot_str[16], own_str[16];
            snprintf(age_str, 16, "%d", age);
            snprintf(ovr_str, 16, "%d", overall);
            snprintf(pot_str, 16, "%d", potential);
            snprintf(own_str, 16, "%d", own);
            
            char* args[6] = {name, age_str, ovr_str, pot_str, own_str, NULL};
            new_player(args);
        }
        else if (strncmp(line, "FORMATION:", 10) == 0) {
            /* Parse formation: FORMATION:name */
            char name[128];
            if (sscanf(line + 10, "%127s", name) != 1) {
                fprintf(stderr, "Warning: Invalid formation line %d\n", line_num);
                continue;
            }
            
            /* Create formation if it doesn't exist */
            char* args[2] = {name, NULL};
            new_formation(args, NULL);
        }
        else if (strncmp(line, "POSITION:", 9) == 0) {
            /* Parse position: POSITION:formation_name:position_id */
            char form_name[128];
            int pos_id;
            
            if (sscanf(line + 9, "%127[^:]:%d", form_name, &pos_id) != 2) {
                fprintf(stderr, "Warning: Invalid position line %d\n", line_num);
                continue;
            }
            
            /* Open the formation and add position */
            char* open_args[2] = {form_name, NULL};
            if (open_formation(open_args, NULL) == 0) {
                char* pos_name = get_position_name(pos_id);
                if (pos_name) {
                    char* add_args[2] = {pos_name, NULL};
                    add_position_to_formation(add_args, NULL);
                }
            }
        }
        else if (strncmp(line, "ASSIGN:", 7) == 0) {
            /* Parse assignment: ASSIGN:formation_name:position_id:player_name */
            char form_name[128], player_name[128];
            int pos_id;
            
            if (sscanf(line + 7, "%127[^:]:%d:%127s", 
                       form_name, &pos_id, player_name) != 3) {
                fprintf(stderr, "Warning: Invalid assign line %d\n", line_num);
                continue;
            }
            
            /* Open formation and assign player */
            char* open_args[2] = {form_name, NULL};
            if (open_formation(open_args, NULL) == 0) {
                char* pos_name = get_position_name(pos_id);
                if (pos_name) {
                    char* add_args[3] = {pos_name, player_name, NULL};
                    add_player_to_position(add_args);
                }
            }
        }
    }
